...
export default function PeerToPeer({}) {
  ...
  const socket = SocketIOClient('http://192.168.1.12:3500', {
  ...
  useEffect(() => {
    ...
    socket.on('callAnswered', data => {
        remoteRTCMessage.current = data.rtcMessage;
        if (peerConnection.current && remoteRTCMessage.current) {
          const { sdp, type } = remoteRTCMessage.current;
          const sessionDescription = new RTCSessionDescription({ sdp, type });
          ...
          console.error('Invalid remoteRTCMessage or peerConnection');
        }
      });
